...
def get_nfl(team):
    ...
    response = requests.get('https://api.sportsdata.io/v3/nfl/scores/json/Schedules/2023', headers=headers)
    data = response.json()
    print({'events': [nfl_dict(event) for event in data if "BYE" not in event["AwayTeam"] and (event["AwayTeam"] == team or event["HomeTeam"] == team)]})
