    ...
    return (
        <div className="formula-container">
            <div className="formula-header">
                ...
                <div className='nba-season'>Season: 24-25</div>
                <div>Coming Soon!</div>
                ...
                {eventsList.length ?
                    eventsList.map((race) => (
                        <div key={`${race.title}-${race.startDate}`} className="formula-item">
                            ...
                            <div>{dayjs(race.startDate).format("dddd MMMM DD, YYYY")} {race.startTime && dayjs(race.startTime).subtract(currentOffset-5, 'hour').format("h:mm a")}</div>
